...
2. 对现有 bev_sem_round2_merged：
   - 先完整读取旧数据；
   - 对本次传入的 run，删除旧版本中的全部同 run 样本；
   - 再加入新的图片和新标签。
   这样 2026-06-28_01-13-14 会被整批替换，不会残留旧的左右反数据。
...
4. 每 10 张中的第 10 张进入 val，其余进入 train：
   - 1~9 train，10 val；
   - 11~19 train，20 val；
   - 依此类推。
...
def main() -> int:
    ...
    print(f"Runs replaced    : {', '.join(args.new_runs)}")
    ...
    try:
        ...
        # 第一步：在每个新 run 内优先 corrected，缺失时使用 masks_id。
        new_pairs = load_new_pairs(
            bev_root=bev_root,
            seg_root=seg_root,
            run_names=args.new_runs,
        )

        # 第二步：读取现有 merged，并删除本次 run 的旧版本。
        existing_pairs = load_existing_pairs(dataset_root)
        merged_pairs, removed_old_count = merge_replacing_runs(
            existing_pairs=existing_pairs,
            new_pairs=new_pairs,
            replaced_runs=set(args.new_runs),
        )

        if len(merged_pairs) < args.val_every:
            raise RuntimeError(
                f"合并后只有 {len(merged_pairs)} 个样本，无法划分验证集。"
            )
        ...
        print(f"New samples added         : {len(new_pairs)}")
        print(
            f"  corrected               : "
            f"{count_masks(new_pairs, 'corrected')}"
        )
        print(
            f"  ONNX fallback           : "
            f"{count_masks(new_pairs, 'onnx_autolabel')}"
        )
